...
function updatePlayer() {
    ...
    const diff = mouseY - playerCenter;
    if (Math.abs(diff) > 5) {
        player.y += diff * 0.1;
...
function updateComputer() {
    ...
    // AI difficulty: lead the ball slightly
    const leadFactor = 0.3;
    const targetY = ball.y + ball.dy * 10 * leadFactor;
    
    if (computerCenter < targetY - 5) {
        computer.y = Math.min(canvas.height - computer.height, computer.y + computer.speed);
    } else if (computerCenter > targetY + 5) {
